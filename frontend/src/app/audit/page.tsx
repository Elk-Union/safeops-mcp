  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold tracking-tight text-slate-800">Cryptographic Audit Ledger</h2>
          <p className="text-slate-500 text-xs mt-1">Tamper-proof history timeline of all agent operations chained using cryptographic row-hash validation.</p>
        </div>
        ...
        <button
          ...
          disabled={verifying}
          className="px-4 py-2 bg-white hover:bg-slate-50 border border-slate-200 hover:border-slate-300 rounded-xl text-xs font-semibold text-slate-700 shadow-sm transition flex items-center gap-2"
        >
          {verifying ? (
            <>
      ...
      {verificationResult && (
        <div className={`p-3.5 border rounded-xl text-xs font-mono font-medium ${
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
        <input
          ...
          className="w-full bg-slate-50 border border-slate-200 hover:border-slate-300 rounded-xl px-4 py-2.5 text-xs text-slate-800 focus:outline-none focus:border-indigo-600 focus:bg-white transition"
        />
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          {loading ? (
            <div className="p-8 text-center text-slate-400 text-xs">Loading ledger blocks...</div>
          ) : filteredLogs.length === 0 ? (
            <div className="p-8 text-center text-slate-400 text-xs">No audit logs matched search query.</div>
          ) : (
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/50 text-slate-400 text-xs font-mono">
                  ...
                  <th className="py-3 px-6 text-right">Cryptographic Row Hash</th>
                </tr>
              </thead>
              <tbody className="text-xs divide-y divide-slate-100">
                {filteredLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-50/40 transition">
                    ...
                    <td className="py-4 px-6 font-bold text-slate-700">{log.client_name}</td>
                    ...
                    <td className="py-4 px-6 text-right font-mono text-[9px] text-slate-400 truncate max-w-[200px]" title={log.row_hash}>
                      {log.row_hash}
                    </td>
                  </tr>
                ))}
